...
// @TODO refactor
var appDir, componentsDir, routesDir, DirectorRouter;
    ...
    function handleRoute () {
      ...
      handler.promise.apply(null, params).then(function (data) {
        ...
        router.renderComponent(handler.render, data, function (err, component) {
          ...
          if (renderer === 'server') {
            var css = router.renderCriticalCSS(handler.criticalCSS);
            var htmlReplace = {
              component: React.renderComponentToString(component),
              componentPath: JSON.stringify(handler.render),
              data: JSON.stringify(data),
              criticalCSS: css.critical,
              nonCriticalCSS: css.nonCritical
            };

            router.handleServerRoute(htmlReplace, routeContext.req, routeContext.res);
          } else {
 ...
 * @param {object} data
 * @param {function} callback (err, component)
 */
...
/**
 * react does magic to display compiled component
 */
...
Router.prototype.serverToClient = function (componentPath, data) {
  ...
  if (self.renderer === 'client') {
    self.renderComponent(componentPath, data, function (err, component) {
      if (err) {
        self.handleErr(err);
      } else {
        self.handleClientRoute(component);
      }
    });
  }
};
...
Router.prototype.handleServerRoute = function (htmlReplace, req, res) {
  try {
